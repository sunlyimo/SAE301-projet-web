        ...
        <div class="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden">
            <div class="p-6">
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">Modifier le Raid</h1>
                        <p class="text-sm text-gray-600">Modification de l'événement et informations de contact</p>
                    </div>
                </div>
                    @if($errors->any())
                    ...
                    @endif
                    <form action="{{ route('raids.update', $raid->RAI_ID) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            ...
                            <div>
                                <div class="p-4 border rounded-lg bg-gray-50">
                                    ...
                                    <div class="mt-2">
                                        ...
                                        <input type="datetime-local" name="RAI_RAID_DATE_DEBUT" value="{{ old('RAI_RAID_DATE_DEBUT', $raid->RAI_RAID_DATE_DEBUT->format('Y-m-d\TH:i')) }}" class="mt-1 block w-full rounded-md border-gray-200 p-2">
                                    </div>
                                    <div class="mt-3">
                                        <label class="block text-sm text-gray-600">Fin</label>
                                        <input type="datetime-local" name="RAI_RAID_DATE_FIN" value="{{ old('RAI_RAID_DATE_FIN', $raid->RAI_RAID_DATE_FIN->format('Y-m-d\TH:i')) }}" class="mt-1 block w-full rounded-md border-gray-200 p-2">
                                    </div>
                                </div>
                            </div>
                            <div>
                                <div class="p-4 border rounded-lg bg-gray-50">
                                    ...
                                    <div class="mt-2">
                                        ...
                                        <input type="datetime-local" name="RAI_INSCRI_DATE_DEBUT" value="{{ old('RAI_INSCRI_DATE_DEBUT', $raid->RAI_INSCRI_DATE_DEBUT->format('Y-m-d\TH:i')) }}" class="mt-1 block w-full rounded-md border-gray-200 p-2">
                                    </div>
                                    <div class="mt-3">
                                        <label class="block text-sm text-gray-600">Clôture</label>
                                        <input type="datetime-local" name="RAI_INSCRI_DATE_FIN" value="{{ old('RAI_INSCRI_DATE_FIN', $raid->RAI_INSCRI_DATE_FIN->format('Y-m-d\TH:i')) }}" class="mt-1 block w-full rounded-md border-gray-200 p-2">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid gap-4 md:grid-cols-2 mb-4">
                            ...
                            <div>
                                ...
                                <input list="responsable_list" id="responsable_input" name="responsable_name" class="mt-1 block w-full rounded-md border-gray-200 p-3" placeholder="Rechercher un membre..." autocomplete="off" value="{{ old('responsable_name', $raid->responsable ? $raid->responsable->UTI_PRENOM . ' ' . $raid->responsable->UTI_NOM : '') }}">
                        ...
                        <div class="mb-5">
                            ...
                            <div class="mt-2">
                                ...
                                @if($raid->RAI_IMAGE)
                                    <p class="text-sm text-gray-500 mt-1">Image actuelle : {{ $raid->RAI_IMAGE }}</p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="grid gap-3 md:grid-cols-3">
                            <div class="md:col-span-2">
                                <button type="submit" class="w-full bg-gray-900 hover:bg-green-600 text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center gap-2">
                                    <i class="fas fa-save"></i> Modifier le Raid
                                </button>
                            </div>
                            <div>
                                <a href="{{ route('raids.courses', $raid->RAI_ID) }}" class="w-full inline-block text-center border border-gray-300 text-gray-700 py-3 rounded-lg">Annuler</a>
                            </div>
                        </div>
                    </form>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Initialize on load
    applyContactForName(input.value.trim());
});
</script>
